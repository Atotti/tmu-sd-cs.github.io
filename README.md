...
5. `_data/staff.yml`ファイルに教員情報を追加します：
...
新しい画像を追加する際は、ウェブ用に最適化することをお勧めします：

1. 適切なサイズにリサイズする（大きすぎる画像はパフォーマンスに影響します）
2. 画像を圧縮する（PNGの場合はpngquant、JPGの場合はjpegoptimなどのツールを使用）
3. 適切なalt属性を追加する（アクセシビリティのため）

### パフォーマンスの最適化

1. 大きなファイルや不要なファイルをコミットしないようにする
2. CSSとJavaScriptを最小化する
3. 画像を最適化する

### バックアップ

定期的にリポジトリのバックアップを作成することをお勧めします：

```bash
git clone --mirror https://github.com/TMU-SD-CS/tmu-sd-cs.github.io.git backup-tmu-sd-cs
```

### 更新履歴の管理

サイトの重要な更新履歴を記録しておくと、将来の管理者にとって役立ちます。

---

このREADMEに関する質問や提案がある場合は、リポジトリのIssueを作成するか、管理者に直接お問い合わせください。
